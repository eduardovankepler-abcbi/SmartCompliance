    ...
    <section class="pdi-dashboard" aria-labelledby="pdi-dashboard-title">
      ...
      @if (overview(); as currentOverview) {
        ...
        <section class="pdi-dashboard__grid">
          ...
          <article class="pdi-dashboard__panel pdi-dashboard__panel--wide">
            <header><div><span>Competências</span><h2>Evolução entre os dois últimos períodos</h2></div><small>mínimo de {{ analytics().minimumAggregateSize }} pessoas</small></header>
            @if (competencyItems().length) {
              <app-dashboard-bar-chart [items]="competencyItems()" ariaLabel="Conclusão de PDIs por competência" [valueMax]="100" />
              <div class="pdi-dashboard__competencies">@for (item of analytics().competencyEvolution; track item.competencyId) { <div><strong>{{ item.competencyName }}</strong><span>{{ item.previousPercentage }}% → {{ item.currentPercentage }}%</span><small [class.negative]="item.delta < 0">{{ deltaLabel(item.delta) }} · {{ item.peopleCount }} pessoas</small></div> }</div>
...
export class DashboardPdiPageComponent implements OnInit {
  ...
  readonly competencyItems = computed<DashboardChartDatum[]>(() => this.analytics().competencyEvolution.map((item) => ({ label: item.competencyName, value: item.currentPercentage, valueLabel: `${item.currentPercentage}%` })));
  ...
  changeArea(area: string): void { if (this.canFilterByArea()) { this.areaFilter.set(area); void this.loadOverview(); } }
  ...
  async loadOverview(): Promise<void> {
    ...
    try {
      const overview = await firstValueFrom(this.dashboardService.getOverview({ area: this.canFilterByArea() && this.areaFilter() !== 'all' ? this.areaFilter() : null, timeGrouping: this.timeGrouping() }));
      if (requestId === this.requestId) this.overview.set(overview);
    } catch (error) {
