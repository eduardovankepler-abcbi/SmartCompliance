...
export class DashboardPdiPageComponent implements OnInit {
  ...
  readonly overdueActionsOnly = signal(false);
  ...
  readonly filteredPriorityActions = computed(() => this.analytics().priorityActions.filter((action) => (this.actionStatusFilter() === 'all' || action.progressStatus === this.actionStatusFilter()) && (!this.overdueActionsOnly() || action.overdue)));
