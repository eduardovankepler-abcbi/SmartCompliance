...
@Component({
  ...
  template: `
    <section class="incidents" aria-labelledby="incidents-title">
      <header class="incidents__header"><div><p>Compliance</p><h1 id="incidents-title">Incidentes</h1><span>{{ canTreat() ? 'Registre relatos e acompanhe a fila dentro do seu escopo.' : 'Use este canal para registrar relatos de etica, conduta e compliance.' }}</span></div><button (click)="openCreate()">Novo relato</button></header>
      ...
      @if (creating()) { <app-incident-create-form [form]="form" [areas]="areas()" [people]="people()" [saving]="saving()" [showValidation]="showCreateValidation()" (submitted)="create()" (cancelled)="cancelCreate()" /> }
      @if (canTreat()) {
        ...
        <article><div class="row"><strong>{{ incident.protocol }} · {{ incident.title }}</strong><span>{{ incident.status }}</span></div><p>{{ incident.category }} · {{ incident.classification }}</p><p>{{ incident.description }}</p><small>Area responsavel: {{ incident.responsibleArea }} · Responsavel: {{ incident.assignedTo }} · Envolvido: {{ subjectName(incident.subjectPersonId) || 'A definir' }} · Procedencia: {{ findingLabel(incident.findingStatus) }}</small><small>Abertura: {{ incident.createdAt | date:'short' }} · Prazo: {{ incident.dueAt | date:'short' }}</small>
          ...
          @if (canTreat()) { <div class="case-timeline"><strong>Historico do caso</strong>@for (entry of auditFor(incident.id); track entry.id) { <small>{{ entry.createdAt | date:'short' }} · {{ entry.summary }} · {{ entry.actorName }}</small> }@if (!auditFor(incident.id).length) { <small>Sem eventos específicos carregados.</small> }</div> }
        ...
        <p class="state">Seu relato sera encaminhado para triagem da area responsavel. A fila de tratamento e a trilha de auditoria ficam restritas aos perfis autorizados.</p>
      }
    </section>
  `,
  styles: `.incidents{max-width:1040px}.incidents__header,.row,.error,.queue header,.evidences>div{display:flex;align-items:start;justify-content:space-between;gap:16px}.incidents__header p{margin:0;color:var(--abc-blue);font-weight:700;text-transform:uppercase;font-size:13px}h1{margin:4px 0}.incidents__header span,.state,article p,article small,.queue header span,.evidences span{color:var(--abc-text-muted)}button{padding:9px 12px;background:var(--abc-blue);color:var(--abc-on-blue);border:0;border-radius:var(--abc-radius);font-weight:700}.secondary{background:var(--abc-surface);color:var(--abc-text);border:1px solid var(--abc-border)}.error{margin-top:20px;padding:12px;color:var(--abc-danger);background:color-mix(in srgb, var(--abc-danger) 8%, var(--abc-surface));border:1px solid color-mix(in srgb, var(--abc-danger) 24%, var(--abc-border));border-radius:8px}.queue{display:grid;gap:14px;margin-top:16px;padding:16px;background:var(--abc-surface);border:1px solid var(--abc-border);border-radius:8px}.queue h2{margin:0;color:var(--abc-text);font-size:18px}.queue article{padding:14px;background:var(--abc-surface);border:1px solid var(--abc-border);border-radius:8px;color:var(--abc-text);box-shadow:0 8px 24px color-mix(in srgb, var(--abc-navy) 6%, transparent)}.queue article p{margin:10px 0;color:var(--abc-text)}.queue article small{display:block;color:var(--abc-text-muted)}.queue article .row span{padding:5px 10px;color:color-mix(in srgb, var(--abc-danger) 28%, var(--abc-surface));background:color-mix(in srgb, var(--abc-danger) 55%, var(--abc-navy));border-radius:999px;font-size:12px}.queue article>.secondary{margin-top:12px;background:var(--abc-surface);color:var(--abc-text);border-color:var(--abc-border)}.evidences,.readiness,.case-timeline{display:grid;gap:8px;margin-top:12px;padding:12px;border:1px solid var(--abc-border);border-radius:8px;background:var(--abc-surface-muted)}.readiness{grid-template-columns:repeat(2,minmax(0,1fr))}.readiness strong{grid-column:1/-1}.readiness span{padding:6px 8px;border:1px solid var(--abc-border);border-radius:6px;color:var(--abc-text-muted)}.readiness span.done{color:var(--abc-text);border-color:color-mix(in srgb,var(--abc-blue) 45%,var(--abc-border))}.evidences label{display:grid;gap:5px}.evidences input{max-width:100%}.link{width:max-content;max-width:100%;padding:0;background:transparent;color:var(--abc-blue);text-align:left;overflow-wrap:anywhere}.state{margin-top:24px}@media(max-width:720px){.incidents__header,.row,.error,.queue header,.evidences>div{display:grid}.readiness{grid-template-columns:1fr}.link{width:100%}}`,
})
export class IncidentsPageComponent implements OnInit {
  ...
  readonly caseAudit = signal<Record<string, AuditEntry[]>>({});
  ...
  readonly showCreateValidation = signal(false);
  ...
  openCreate(): void {
    ...
    this.showCreateValidation.set(false);
  ...
  cancelCreate(): void { this.creating.set(false); this.showCreateValidation.set(false); this.form.reset(); }
  async create(): Promise<void> {
    this.showCreateValidation.set(true);
  ...
  readinessFor(incident: Incident): Array<{ label: string; done: boolean }> {
    return [
      { label: 'Responsavel definido', done: Boolean(incident.assignedPersonId || incident.assignedTo !== 'Nao definido') },
      { label: 'Evidencia anexada', done: this.evidencesFor(incident.id).length > 0 },
      { label: 'Historico registrado', done: this.auditFor(incident.id).length > 0 },
      { label: 'Motivo de conclusao', done: incident.status !== 'Concluido' || Boolean(incident.closureNote) }
    ];
  }
  async uploadEvidence(incident: Incident, file: File | undefined, input: HTMLInputElement): Promise<void> {
    ...
    try { await firstValueFrom(this.api.addEvidence(incident.id, file)); input.value = ''; await this.loadEvidences(this.incidents()); await this.loadCaseAudit(this.incidents()); await this.loadAuditTrail(); }
  ...
  private async loadCaseAudit(incidents: Incident[]): Promise<void> {
    if (!this.canTreat() || !incidents.length) { this.caseAudit.set({}); return; }
    const entries = await Promise.all(incidents.map(async (incident) => [incident.id, await firstValueFrom(this.auditApi.list('incident', 8, { entityType:'incident', entityId:incident.id }))] as const));
    this.caseAudit.set(Object.fromEntries(entries));
  }
  ...
  async load(): Promise<void> {
    ...
    try {
      if (!this.canTreat()) {
        const areas = await firstValueFrom(this.areasApi.list());
        this.incidents.set([]); this.areas.set(areas); this.people.set([]); this.auditEntries.set([]); this.evidences.set({}); this.caseAudit.set({});
        return;
      }
      const data = await firstValueFrom(forkJoin({ incidents:this.api.list(), areas:this.areasApi.list(), people:this.peopleApi.list(), audit:this.auditApi.list('incident') }));
      this.incidents.set(data.incidents); this.areas.set(data.areas); this.people.set(data.people); this.auditEntries.set(data.audit); await this.loadEvidences(data.incidents); await this.loadCaseAudit(data.incidents);
    }
