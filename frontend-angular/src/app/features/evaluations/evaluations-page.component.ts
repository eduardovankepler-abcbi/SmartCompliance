...
@Component({
  ...
  template: `
    <section class="evaluations" aria-labelledby="evaluations-title">
      ...
      <label class="module-select">Modalidade<select [value]="activeModule()" (change)="onModuleChange($any($event.target).value)">@for(module of modules; track module){<option [value]="module">{{ relationshipLabel(module) }} ({{ assignmentCountByModule(module) }})</option>}</select></label>
...
export class EvaluationsPageComponent implements OnInit {
  ...
  relationshipLabel(value: string): string { return ({ self:'Autoavaliação profissional', 'leader-self':'Autoavaliação do líder', manager:'Avaliação do líder sobre o colaborador', peer:'Par', 'peer-same-area':'Colega da mesma área', leader:'Avaliação do colaborador sobre o líder', company:'Empresa', 'cross-functional':'Avaliação por Colaborador de Outro Setor', 'client-internal':'Cliente interno', 'client-external':'Cliente externo' } as Record<string,string>)[value] || value; }
  ...
  onModuleChange(value: string): void { if (evaluationModules.includes(value as EvaluationModule)) void this.goToModule(value as EvaluationModule); }
