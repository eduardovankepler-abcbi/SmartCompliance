...
export function createUsersRouter(store) {
  ...
  router.get("/", requireRoles(...PERMISSIONS.userRegistry), async (req, res, next) => {
    try {
  ...
  router.post("/", requireRoles(...PERMISSIONS.userRegistry), async (req, res) => {
  ...
  router.patch("/:userId", requireRoles(...PERMISSIONS.userRegistry), async (req, res) => {
